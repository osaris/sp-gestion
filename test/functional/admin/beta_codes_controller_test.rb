...
class Admin::BetaCodesControllerTest < ActionController::TestCase
  ...
  context "an user logged in and HTTP_REFERER set" do
    ...
    context "requesting GET :boost_activation which send boost" do
      setup do
        BetaCode.any_instance.stubs(:boost_activation).returns(true)
        get :boost_activation, :id => BetaCode.make.id
      end
            
      should_respond_with(:redirect)
      should_redirect_to("newsletters list") { "/admin/beta_codes" }
    ...
    context "requesting GET :boost_activation which fail" do
      setup do
        BetaCode.any_instance.stubs(:boost_activation).returns(false)
        get :boost_activation, :id => BetaCode.make.id
      end
